    ...
    if (loop_mode)
    {
        ...
        case CALLCONV_MICROSOFT_X64:
            if (target_num_args > 0)
                saved_args[0] = (void *)lcContext.Rcx;
            if (target_num_args > 1)
                saved_args[1] = (void *)lcContext.Rdx;
            if (target_num_args > 2)
                saved_args[2] = (void *)lcContext.R8;
            if (target_num_args > 3)
                saved_args[3] = (void *)lcContext.R9;
        ...
        case CALLCONV_FASTCALL:
            if (target_num_args > 0)
                saved_args[0] = (void *)lcContext.Ecx;
            if (target_num_args > 1)
                saved_args[1] = (void *)lcContext.Edx;
            if (target_num_args > 3)
            {
...
void Debugger::HandleTargetEnded()
{
    ...
    if (loop_mode)
    {
        ...
        case CALLCONV_MICROSOFT_X64:
            if (target_num_args > 0)
                lcContext.Rcx = (size_t)saved_args[0];
            if (target_num_args > 1)
                lcContext.Rdx = (size_t)saved_args[1];
            if (target_num_args > 2)
                lcContext.R8 = (size_t)saved_args[2];
            if (target_num_args > 3)
                lcContext.R9 = (size_t)saved_args[3];
            if (target_num_args > 4)
            {
        ...
        case CALLCONV_FASTCALL:
            if (target_num_args > 0)
                lcContext.Ecx = (size_t)saved_args[0];
            if (target_num_args > 1)
                lcContext.Edx = (size_t)saved_args[1];
            if (target_num_args > 3)
            {
